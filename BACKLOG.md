...
### TEST-11 · No Recall@K or MRR tracking for the retriever
**Files:** `tests/test_rag_quality.py`, `app/rag/retriever.py`  
The current RAGAS evaluation measures `ContextRecall` (LLM-judged) but does not track standard retrieval metrics: **Recall@K** (what fraction of relevant chunks were retrieved in the top K results) and **MRR** (Mean Reciprocal Rank — how high up the first relevant chunk ranks). Without these, it is impossible to tell whether changing the embedding model, chunk size, or k value improves or degrades retrieval independently of LLM answer quality.  
Fix: Add a retrieval-only evaluation step in `test_rag_quality.py` that for each test case: (1) defines which chunk(s) are relevant by section name, (2) calls `retriever.invoke(question)`, (3) computes Recall@3 and Reciprocal Rank, (4) asserts mean Recall@3 >= 0.8 and MRR >= 0.7. This makes retriever quality independently measurable — useful when tuning k or switching embedding models.
